...
/// Why a rendered document could not be spliced before sanitization.
#[derive(Debug)]
...
impl RenderedMarkdown {
    ...
    /// Splice generated markup only when every placeholder occurs exactly
    /// once in the raw renderer output. Validation happens for the complete
    /// set before the first replacement so a collision can never cause a
    /// partial splice. Callers sanitize the completed markup afterward, so
    /// no generated markup is inserted beyond the sanitizer boundary.
...
fn highlight_css() -> &'static str {
    ...
    CSS.get_or_init(|| {
        ...
        format!(
            ...
            scope_theme_css(&light, ":root"),
            scope_theme_css(&dark, ":root"),
...
mod tests {
    ...
    #[test]
    fn generated_theme_css_has_no_js_defaults_and_manual_palettes() {
        let css = highlight_css();
        assert!(css.contains(":root .mdo-syntect-code"));
        assert_eq!(css.matches(":root .mdo-syntect-code").count(), 2);
        ...
        assert!(
            css.find("@media (prefers-color-scheme: dark)")
                < css.find(":root[data-theme=\"light\"]")
        );
        ...
        for line in css.lines().filter(|line| line.contains(".mdo-syntect-")) {
            ...
            for selector in selector_list.split(',') {
                ...
                assert!(
                    selector.starts_with(":root .mdo-syntect-")
